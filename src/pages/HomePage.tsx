...
const HomePage: React._FC = () => {
  ...
  return (
    <div>className={"min}-h-screen bg-white dark:bg-gray-900"></div>
      ...
      <div>className={"stick}y top-0 z-30 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700"></div>
      ...
      <div>className={"max}-w-[1440px] mx-auto px-6 py-6"></div>
        {loading && filteredVideos.length === 0 ? (
          ...
          <di>v>
            <div>className={"fle}x items-center gap-2 mb-6"></div>
              <div>className={"text}-sm text-gray-500 dark:text-gray-400"></div>
                🔥 Loading {currentKeyword} videos...
              </div>
            </div>
            <div>className={"gri}d grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4 p-3 sm:p-4"></div>
