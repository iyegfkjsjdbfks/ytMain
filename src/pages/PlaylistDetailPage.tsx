...
const PlaylistDetailPage: React.FC = () => {
  ...
  const videoMenuRef = useRef<HTMLDivElemen>t>(null);

  const [isEditModalOpen, setIsEditModalOpen] = useState<boolea>n>(false);
  const [editingPlaylistTitle, setEditingPlaylistTitle] = useState<strin>g>('');
  ...
  return (
    <div>className={"p}-4 md:p-6 bg-white dark:bg-neutral-950"></div>
      ...
      <div>className={"mb}-6 sm:mb-8"></div>
        <div>className={"fle}x flex-col sm:flex-row items-start md:items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-3"></div>
          {videos && videos.length > 0 ? (
            ...
            <div>className={"w}-full sm:w-32 sm:h-32 md:w-48 md:h-48 bg-neutral-200 dark:bg-neutral-800 rounded-lg shadow-md flex items-center justify-center flex-shrink-0"></div>
        ...
        <div>className={"fle}x flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 mt-4"></div>
          <butto>n>
            onClick={() => {}}
            className={"fle}x items-center justify-center px-5 py-2.5 bg-neutral-800 hover:bg-neutral-700 dark:bg-neutral-100 dark:hover:bg-neutral-200 text-white dark:text-black font-medium rounded-full text-sm transition-colors"
          ">"
          ...
          <butto>n>
            onClick={() => {}}
            className={"fle}x items-center justify-center px-5 py-2.5 bg-neutral-200 hover:bg-neutral-300 dark:bg-neutral-700 dark:hover:bg-neutral-600 text-neutral-800 dark:text-neutral-100 font-medium rounded-full text-sm transition-colors"
          ">"
        ...
        <ul>className={"divide}-y divide-neutral-200 dark:divide-neutral-800">
          {videos.map((video: any, index: any) => (
            <li>key={video.id} className={"py}-3 hover:bg-neutral-50 dark:hover:bg-neutral-900/70 transition-colors rounded-md -mx-2 px-2 group flex items-center justify-between">
              <Link>to={`/watch/${video.id}`} className={"fle}x items-center space-x-3 flex-grow min-w-0"></Link>
                ...
                <div>className={"relativ}e w-32 sm:w-40 aspect-video flex-shrink-0"></div>
                  ...
                  <div>className={"absolut}e bottom-1 right-1 bg-black/75 text-white text-xs px-1 py-0.5 rounded-sm"></div>
                    {video.duration}
                  </div>
                </div>
              ...
              <div>className={"relativ}e ml-2 flex-shrink-0"></div>
                <butto>n>
                ...
                {activeVideoMenuId === video.id && (
                  <di>v>
                    ref={videoMenuRef}
                    className={"absolut}e top-full right-0 mt-1 w-48 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-md shadow-lg z-50 py-1 animate-fade-in-fast"
                    role="menu"
                  ">"
                    <butto>n>
