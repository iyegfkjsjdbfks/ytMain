...
const CommentModerationPage: React.FC<CommentModerationPageProp>s> = ({ className }) => {
  ...
  return (
    <div>className={`comment-moderation-page ${className || ''}`}></div>
      <div>className={"containe}r mx-auto px-4 py-8"></div>
        <h1>className={"text}-3xl font-bold mb-6">Comment Moderation</h1>
        
        {/* Stats Overview */}
        <div>className={"gri}d grid-cols-1 md:grid-cols-4 gap-6 mb-8"></div>
...
{filter !== 'all' && (
            ...
            {filteredComments.length === 0 ? (
              ...
              filteredComments.map((comment: any) => (
                <div>key = {comment.id} className = "border rounded-lg p-4"></div>
                  <div>className={"fle}x justify-between items-start mb-3"></div>
                    <div>className={"flex}-1"></div>
                      <div>className={"fle}x items-center space-x-3 mb-2"></div>
                  ...
                  <div>className={"bg}-gray-50 rounded p-3 mb-4"></div>
      ...
      className={"w}-full border rounded p-2 text-sm"
              ...
              /">"
              <button>className={"mt}-2 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 text-sm"></button>
                Save Blocked Words;
              </button>
            </div>
          </div>
