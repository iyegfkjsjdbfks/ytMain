...
const StudioVideoGrid: React.FC<StudioVideoGridProps> = ({
  ...
  const handleDeleteSelected = () => {
    if (selectedVideos.size === 0 || !onDelete) return;
      ...
      {sortedVideos.length > 0 ? (
        <div className="divide-y">
          {sortedVideos.map(video => (
            <div key={video.id} className="flex items-center p-4 hover:bg-gray-50">
              <input 
                type="checkbox" 
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center flex-grow">
                <div className="flex items-center flex-grow min-w-0">
                  ...
                  <div className="ml-3 min-w-0 flex-grow">
                    <Link to={`/studio/videos/edit/${video.id}`} className="block hover:underline">
