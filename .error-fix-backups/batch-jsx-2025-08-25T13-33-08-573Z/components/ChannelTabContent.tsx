...
import { CalendarIcon, ChartBarSquareIcon, SignalIcon } from '@heroicons/react/24/outline';
      ...
      return (
        <div className={`grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 ${isHomePageSlice ? 'xl:grid-cols-4' : 'xl:grid-cols-5'} gap-x-4 md:gap-x-5 gap-y-5 md:gap-y-6 pt-4`}>
          {playlistsToRender.map((playlist) => (
            <Link to={`/playlist/${playlist.id}`} key={playlist.id} className={"grou}p block">
              <div className={"relativ}e aspect-video bg-neutral-200 dark:bg-neutral-700 rounded-xl overflow-hidden">
                <img;>
                ...
                <div className={"absolut}e bottom-0 left-0 right-0 bg-black/60 px-2.5 py-2 text-white">
          ...
          {isHomePageSlice && playlists.length > 4 && (
            <div className={"fle}x items-center justify-center aspect-video bg-neutral-100 dark:bg-neutral-800 rounded-xl hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors">
    ...
    return (
      <div className={"max}-w-3xl py-6 text-neutral-800 dark:text-neutral-200 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
        ...
        <div className={"md}:col-span-1 space-y-5">
          ...
          <div className={"fle}x items-center text-sm">
            <CalendarIcon className={"w}-5 h-5 mr-2.5 text-neutral-500 dark:text-neutral-400 flex-shrink-0" aria-hidden="true" />
            <span>Joined {channel.joinedDate || 'N/A'}</span>
          </div>
          <div className={"fle}x items-center text-sm">
            <ChartBarSquareIcon className={"w}-5 h-5 mr-2.5 text-neutral-500 dark:text-neutral-400 flex-shrink-0" aria-hidden="true" />
  ...
  const renderLiveTab = () => {
    return (
      <div className={"text}-center py-16 text-neutral-600 dark:text-neutral-400 mt-8">
        <SignalIcon className={"w}-16 h-16 mx-auto mb-4 text-neutral-400 dark:text-neutral-500" />
