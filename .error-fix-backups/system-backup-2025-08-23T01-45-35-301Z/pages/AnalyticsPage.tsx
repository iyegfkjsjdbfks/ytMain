...
const AnalyticsPage: React.FC = () => {}
 ...
 useEffect(() => {}
 const fetchAnalyticsData = async (): Promise<any> < void> => {}
 setLoading(true);
 try {}
 ...
 recentPerformance: {,}
 views: Array<any>.from({ length: 30 }, () => Math.floor(Math.random() * 10000) + 1000),
 watchTime: Array<any>.from({ length: 30 }, () => Math.floor(Math.random() * 500) + 100),
 subscribers: Array<any>.from({ length: 30 }, () => Math.floor(Math.random() * 100) + 10),
 labels: Array<any>.from({ length: 30 }, (_, i) => {}
 const date = new Date();
