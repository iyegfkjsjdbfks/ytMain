...
const MIN_DESC_LENGTH_FOR_SUMMARY: number = 100;
const RELATED_VIDEOS_INITIAL_COUNT: number = 10;
...
export const useWatchPage = () => {
 ...
 // Load video data
 useEffect(() => {
 const loadVideoData = async (): Promise<any> < void> => {
 if (!videoId) {
 return;
 }
 ...
 const handleSaveToPlaylist = async (playlistId): Promise<any> < any> => {
 ...
 const handleCreatePlaylist = async (name, description?: string): Promise<any> < any> => {
 ...
 const handleSummarizeDescription = async (): Promise<any> < void> => {
 ...
 const handleEditSave = (commentId: string | number, newText: string | number, parentId?: string) => {
 if (!newText.trim()) {
 return;
 }
 ...
 const handleDeleteComment = (commentId: string | number, parentId?: string) => {
 if (
 !window.confirm(
 ...
 const toggleLikeDislikeForCommentOrReply = (, id: string | number, parentId: string | undefined, action: 'like' | 'dislike') => {
