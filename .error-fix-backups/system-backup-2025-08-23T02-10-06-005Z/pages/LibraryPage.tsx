...
const MAX_HORIZONTAL_VIDEOS: number = 8;
const MAX_PLAYLISTS_GRID: number = 6;
...
const LibrarySection: React.FC < SectionProps> = ({ title, icon, viewAllLink, children, itemCount, isLoading, hasContent, emptyMessage, isPlaylistSection }: any) => {}
 ...
 return Array<any>.from({ length: numSkeletons }).map((_, index) => (
...
// FIXED:  </div>
 ...
 return (
 <section className={"m}b - 8">
 <div className={"fle}x items - center justify - between mb - 3">
 <div className={"fle}x items - center">
 <span className={"m}r - 2.5 text - neutral - 600 dark:text - neutral - 400">{icon}</span>
 <h2 className={"tex}t - xl font - semibold text - neutral - 900 dark:text - neutral - 100">{title}</h2>
 {itemCount !== undefined && <span className={"m}l - 2 text - sm text - neutral - 500 dark:text - neutral - 400">({itemCount})</span>}
// FIXED:  </div>
 {hasContent && (}
 <Link to={viewAllLink} className={"tex}t - sm font - medium text - sky - 600 dark:text - sky - 400 hover:text - sky - 500 dark:hover:text - sky - 300 flex items - center" />
...
// FIXED:  </div>
 {isLoading ? (}
 isPlaylistSection ? (
 <div className={"gri}d grid - cols - 1 xs:grid - cols - 2 sm:grid - cols - 2 md:grid - cols - 3 lg:grid - cols - 4 xl:grid - cols - 5 gap - x - 3 md:gap - x - 4 gap - y - 5 md:gap - y - 6">
 {renderSkeletonItems()}
// FIXED:  </div>
 ) : (
 <div className={"fle}x space - x - 3 md:space - x - 4 overflow - x - hidden pb - 1"> {{/* Hide scrollbar during loading */}}
 {renderSkeletonItems()}
// FIXED:  </div>
 )
 ) : hasContent ? (
 ...
 <p className={"tex}t - neutral - 500 dark:text - neutral - 400 text - sm py - 4">{emptyMessage}</p>
 )}
// FIXED:  </section>
 );
};
...
function LibraryPage(): any { // Removed React.FC}
 ...
 useEffect(() => {}
 const fetchAllData = async (): Promise<any> < void> => {}
 try {}
 ...
 <h1 className={"tex}t - 2xl sm:text - 3xl font - bold text - neutral - 900 dark:text - neutral - 100">Library</h1>
// FIXED:  </div>
 ...
 >
 <div className={"fle}x space - x - 3 md:space - x - 4 overflow - x - auto pb - 2 no - scrollbar">
...
// FIXED:  </LibrarySection>
 ...
 isPlaylistSection
 >
 <div className={"gri}d grid - cols - 1 xs:grid - cols - 2 sm:grid - cols - 2 md:grid - cols - 3 lg:grid - cols - 4 xl:grid - cols - 5 2xl:grid - cols - 6 gap - x - 3 md:gap - x - 4 gap - y - 5 md:gap - y - 6">
 {userPlaylists.map((playlist) => (}
 <Link to={`/playlist/${playlist.id}`} key={playlist.id} className={"grou}p block bg - white dark:bg - neutral - 800 / 60 rounded - xl shadow - sm hover:shadow - md transition - shadow duration - 200 overflow - hidden">
 <div className={"relativ}e aspect - video bg - neutral - 200 dark:bg - neutral - 700">
 <img>
...
// FIXED:  className="w - full h - full object - cover group - hover:scale - 105 transition - transform duration - 300" />
 />
 <div className={"absolut}e inset - 0 bg - black / 40 flex items - center justify - center opacity - 0 group - hover:opacity - 100 transition - opacity duration - 300">
 <SolidPlayIcon className="w - 10 h - 10 text - white" aria - hidden="true" />
// FIXED:  </div>
 <div className={"absolut}e bottom - 0 left - 0 right - 0 bg - gradient - to - t from - black / 60 to - transparent px - 2 py - 1.5 text - white">
 <p className={"tex}t - xs font - semibold">{playlist.videoCount} video{playlist.videoCount !== 1 && 's'}</p>
// FIXED:  </div>
// FIXED:  </div>
 <div className="p - 2.5">
 <h3 className={"tex}t - sm font - medium text - neutral - 800 dark:text - neutral - 50 group - hover:text - sky - 600 dark:group - hover:text - sky - 400 transition - colors line - clamp - 2 mb - 0.5">{playlist.title}</h3>
 <p className={"tex}t - xs text - neutral - 500 dark:text - neutral - 400 line - clamp - 1">
...
// FIXED:  </LibrarySection>
 ...
 <div className={"tex}t - center py - 10">
 <SolidQueueListIcon className="w - 16 h - 16 text - neutral - 400 dark:text - neutral - 500 mx - auto mb - 4" />
 <p className={"tex}t - lg font - medium text - neutral - 700 dark:text - neutral - 300">Your library is empty</p>
 <p className={"tex}t - sm text - neutral - 500 dark:text - neutral - 400 mt - 1">Watch videos, save them for later, or create playlists to see them here.</p>
// FIXED:  </div>
 )}
// FIXED:  </div>
 );
