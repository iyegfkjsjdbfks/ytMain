...
function withPerformanceMonitoring < T>(
 queryFn: () => Promise<any> < T>,
 queryKey
): () => Promise<any> < T> {
 return async (): Promise<any> < void> => {
...
export function useEnhancedMutation<;
 ...
 TVariables = void,
>(,
 mutationFn: (variables: TVariables) => Promise<any> < TData>,
 options: {
 maxRetries?: number;
 baseRetryDelay?: number;
 maxRetryDelay?: number;
 ...
 ? async (variables: TVariables): Promise<any> < any> => {
 const startTime = performance.now();
 const endpoint: string = 'mutation'; // Could be enhanced to include specific mutation name
...
export function useInfiniteEnhancedQuery < TData = unknown, _TError = ApiError>(,;
 _queryKey,
 _queryFn: ({ pageParam }: any) => Promise<any> < TData>,
