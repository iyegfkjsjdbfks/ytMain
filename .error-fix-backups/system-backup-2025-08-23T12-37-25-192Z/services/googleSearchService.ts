...
import { googleSearchVideoStore } from 'googleSearchVideoStore.ts';
...
const fetchVideoDetailsInternal = async (videoIds): Promise<any> < Map < string, YouTubeVideoDetails>> => {
...
const fetchChannelDetailsInternal = async (channelIds): Promise<any> < Map < string, YouTubeChannelDetails>> => {
...
export const searchYouTubeVideos = async (query): Promise<any> < YouTubeSearchResult[]> => {
 ...
 const channelIds: any[] = [...new Set(data.items.map((item) => item.snippet.channelId))];

 // Fetch enhanced metadata in parallel
 const [videoDetailsMap, channelDetailsMap] = await Promise<any>.all([
...
export const searchYouTubeWithGoogleSearch = async (query): Promise<any> < GoogleSearchResult[]> => {
 ...
 const channelIds = Array<any>.from(new Set(
 Array<any>.from(videoDetailsMap.values()).map((video) => video.snippet.channelId),
 ));
...
export function extractChannelFromTitle(title: any): string | null {
...
export function generateChannelAvatarUrl(channelName: any): string {
