...
return null;
 ...
 return (
 <div
// FIXED:  className="fixed bottom-4 right-4 z-[999] w-[360px] bg-white dark:bg-neutral-800 rounded-lg shadow-2xl overflow-hidden flex transition-all duration-300 ease-in-out transform animate-slide-in-right-mini border border-neutral-200 dark:border-neutral-700"
 role="complementary"
// FIXED:  aria-label={`Miniplayer for ${video.title}`} />
 >
 <Link
 to={`/watch/${video.id}`} />
// FIXED:  onClick={(e: any) => {
...
// FIXED:  className="text-sm font-medium text-neutral-800 dark:text-neutral-50 hover:text-neutral-600 dark:hover:text-neutral-100 line-clamp-2 leading-tight"
 title={video.title}
 >
 {video.title}
// FIXED:  </Link>
...
// FIXED:  className="text-xs text-neutral-600 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200 block truncate mt-0.5"
...
// FIXED:  className="absolute top-1.5 right-1.5 p-1.5 text-neutral-500 dark:text-neutral-300 hover:text-neutral-800 dark:hover:text-white hover:bg-neutral-200 dark:hover:bg-neutral-700/60 rounded-full transition-colors"
// FIXED:  aria-label="Close miniplayer"
