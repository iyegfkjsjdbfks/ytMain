    ...
    <>
      <div className="h-32 sm:h-48 md:h-56 lg:h-64 bg-neutral-200 dark:bg-neutral-700/30">
        <img
          ...
          alt={`${(channel.name || channel.title)} banner`}
      ...
      <div className="px-4 md:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-end -mt-10 sm:-mt-12 md:-mt-16 mb-4 sm:mb-6 relative z-10">
          <img
            src={(channel.avatarUrl || channel.thumbnail) || ''}
            alt={`${(channel.name || channel.title)} avatar`}
          ...
          <div className="flex-grow mt-3 sm:mt-0 text-center sm:text-left">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-neutral-900 dark:text-neutral-50">{(channel.name || channel.title)}</h1>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
              @{(channel.name || channel.title).toLowerCase().replace(/\s+/g, '')} &bull; {(channel.subscribers || channel.subscriberCount)} &bull; {videoCount} videos
            </p>
          </div>
          <div className="mt-4 sm:mt-0 sm:ml-auto flex-shrink-0">
            <button
              ...
              title={isSubscribed ? `Unsubscribe from ${(channel.name || channel.title)}` : `Subscribe to ${(channel.name || channel.title)}`}
            >
