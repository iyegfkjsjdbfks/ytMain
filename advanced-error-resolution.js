...
export class LiveStreamAPI {
  ...
  async getLiveStreamMetrics(streamId: string): Promise<LiveStreamMetrics> {
    try {
      console.log('Getting live stream metrics:', streamId);
  ...
  async updateLiveStream(streamId: string, updates: Partial<LiveStreamConfig>): Promise<LiveStreamStatus> {
    try {
      console.log('Updating live stream:', streamId, updates);
...
export class MetadataNormalizationService {
  ...
  private normalizeDuration(duration: any): string {
    if (typeof duration === 'string') {
      // Handle ISO 8601 duration (PT4M13S) or simple format (4:13)
  ...
  private normalizeCount(count: any): number {
    ...
    if (typeof count === 'string') {
      // Handle formats like "1.2K", "1.5M", "1,234"
  ...
  normalizeBatch(rawItems: RawVideoMetadata[], source = 'unknown'): NormalizedVideoMetadata[] {
    return rawItems.map(item => this.normalizeVideoMetadata(item, source));
  }
}
