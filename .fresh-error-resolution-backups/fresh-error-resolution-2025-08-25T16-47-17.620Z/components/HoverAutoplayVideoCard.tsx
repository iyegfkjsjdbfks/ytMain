  ...
  return (
    ...
    >
      <Link to={`/watch?v=${video.id}`} className={"block}">
        <div className={"relativ}e bg-gray-100 rounded-lg overflow-hidden aspect-video">
          {/* Thumbnail */}
          <img>
            src={video.thumbnail}
            alt={video.title}
          ...
          {isPlaying && (
            <video>
              ref={videoRef}
              className={"absolut}e inset-0 w-full h-full object-cover"
              muted={isMuted}
          ...
          {/* Duration */}
          <div className={"absolut}e bottom-2 right-2 bg-black bg-opacity-75 text-white text-xs px-1.5 py-0.5 rounded">
            {formatDuration(video.duration)}
          </div>
          
          {/* Video controls overlay */}
          {showControls && isPlaying && (
            <div className={"absolut}e inset-0 bg-black bg-opacity-25 flex items-center justify-center">
              <button>
                onClick={(e) => {
                  e.preventDefault();
                  setIsMuted(!isMuted);
        ...
        <div className={"mt}-3 flex gap-3">
          ...
          <div className={"flex}-1 min-w-0">
            ...
            <div className={"fle}x items-center gap-1 text-sm text-gray-600 mt-0.5">
