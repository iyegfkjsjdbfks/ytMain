...
function createHookIntegrationTemplate(fileName, filePath) {
...
createAdvancedComponentTemplate(componentName) {
...
export const ${componentName}: React.FC<${componentName}Props> = ({
  ...
  const componentClasses = useMemo(() => {
    ...
    return classes.join(' ');
  }, [className, isActive, disabled, loading]);
      ...
      <div className="component-content">
        {children || (
          <div className="default-content">
            <button onClick={() => handleAction('toggle')} disabled={disabled}>
...
function createAdvancedServiceTemplate(serviceName) {
...
on createAdvancedPageTemplate(pageName) {
  ...
  return (
    <div className={'page ' + className}>
      <div className="page-header">
        <h1>{data?.title || pageName.replace(/([A-Z])/g, ' $1').trim()}</h1>
...
ion backupAndRestoreHookIntegration(filePath) {
  if (!fs.existsSync(filePath)) {
