#!/usr/bin/env node
/**
 * Simple TS2304 Error Fixer
 * Applies conservative fixes for TS2304 errors
 */
...
class SimpleTS2304Fixer {
  ...
  async fixError(error) {
    ...
    try {
      ...
      if (result.fixed) {
        ...
        this.fixedCount++;
        this.log('Fixed TS2304 in ' + error.file);
        return true;
      }
  ...
  async run() {
    this.log('🚀 Starting simple TS2304 fixes...');
    ...
    this.log('Found ' + errors.length + ' TS2304 errors to fix');
...
if (import.meta.url.endsWith('fix-ts2304-simple.js')) {
  const fixer = new SimpleTS2304Fixer();
  fixer.run().catch(console.error);
}

export { SimpleTS2304Fixer };
