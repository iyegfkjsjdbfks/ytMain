...
#!/usr/bin/env node
/**
 * Simple TS7006 Error Fixer
 * Applies conservative fixes for TS7006 errors
 */
...
class SimpleTS7006Fixer {
  ...
  getErrors() {
    try {
      ...
      for (const line of lines) {
        const match = line.match(/^(.+?)\((\d+),(\d+)\): error TS7006: (.+)$/);
        if (match) {
  ...
  async fixError(error) {
    ...
    try {
      ...
      if (result.fixed) {
        ...
        this.fixedCount++;
        this.log('Fixed TS7006 in ' + error.file);
        return true;
      }
  ...
  async run() {
    this.log('🚀 Starting simple TS7006 fixes...');
...
if (import.meta.url.endsWith('fix-ts7006-simple.js')) {
  const fixer = new SimpleTS7006Fixer();
  fixer.run().catch(console.error);
}

export { SimpleTS7006Fixer };
