 ...
 * @param seconds - The duration in seconds (can be a float, will be floored)
...
export function getVideoCaptions(video?: Partial<Video>): VideoCaption[] {
  try {
    // In a real app, this would fetch available captions from the video object
    // For now, return a default set of captions
    ...
    if (video?.captions?.length) {
      return video.captions.map((caption: any, index: number) => {
